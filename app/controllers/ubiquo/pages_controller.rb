class Ubiquo::PagesController < UbiquoController
  ...
  def index
    ...
    @pages_pages, @pages = Page.paginate(:page => params[:page], :per_page => params[:per_page]) do
      uhook_find_private_pages(filters, params[:order_by], params[:sort_order])
    end
  ...
  def expire_pages
    if params[:expire_all]
      ...
      if url.present?
        Page.expire_url(url)
        CacheManager::RelatedWidgetPagesExpirer.expire_from_urls(url)
